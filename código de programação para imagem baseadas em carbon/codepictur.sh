...
for id in $(ShellBot.ListUpdates) #roda cada elemento da lista de atualizações uma a uma para ser adicionadas separadamente.   
		...
		(
			...
			printf "$code" | sed "s/$twopoints/$vermelho2 $twopoints<\/a>/" | sed "s/read/$brancolaranj read<\/a>/"  | sed "s/while/$vermelho2 while<\/a>/" | sed "s/echo/$vermelho2 echo<\/a>/" | sed "s/int /$vermelho2 int <\/a>/" | sed "s/true/$vermelho2 true<\/a>/" | sed "s/false/$vermelho2 false<\/a>/" | sed "s/True/$vermelho2 True<\/a>/" | sed "s/False/$vermelho2 False<\/a>/" | sed "s/float/$vermelho2 float<\/a>/" | sed "s/php/$vermelho2 php<\/a>/" | sed "s/function/$vermelho2 function<\/a>/" | sed "s/printf/$vermelho2 printf<\/a>/" | sed "s/\[/$roxo2 \[<\/a>/" | sed "s/\]/$roxo2 \]<\/a>/" | sed "s/(/$roxo2 (<\/a>/" | sed "s/)/$roxo2 )<\/a>/" | sed "s/\[\[/$roxo2 \[\[<\/a>/" | sed "s/\]\]/$roxo2 \]\]<\/a>/" | sed "s/))/$roxo2 ))<\/a>/" | sed "s/esac/$vermelho2 esac<\/a>/" | sed "s/done/$vermelho2 done<\/a>/" | sed "s/;;/$roxo2 ;;<\/a>/" | sed "s/{/$vermelho2 {<\/a>/" | sed "s/}/$vermelho2 }<\/a>/" | sed "s/fi /$vermelho2 fi<\/a>/" | sed "s/if/$vermelho2 if<\/a>/" | sed "s/==/$vermelho2 ==<\/a>/" | sed 's/^/<h3>/' | sed 's/$/<\/h3>/' >> $render.html
			...
			printf "$code" | sed "s/$twopoints/$azulescuro $twopoints<\/a>/" | sed "s/read/$brancolaranj read<\/a>/"  | sed "s/while/$rosa while<\/a>/" | sed "s/echo/$rosa echo<\/a>/" | sed "s/int /$rosa int <\/a>/" | sed "s/true/$rosa true<\/a>/" | sed "s/false/$rosa false<\/a>/" | sed "s/True/$rosa True<\/a>/" | sed "s/False/$rosa False<\/a>/" | sed "s/float/$rosa float<\/a>/"  | sed "s/php/$azulescuro php<\/a>/" | sed "s/function/$rosa function<\/a>/" | sed "s/printf/$rosa printf<\/a>/"  | sed "s/\[/$azulescuro \[<\/a>/" | sed "s/\]/$azulescuro \]<\/a>/" | sed "s/(/$azulescuro (<\/a>/" | sed "s/)/$azulescuro )<\/a>/" | sed "s/\[\[/$azulescuro \[\[<\/a>/" | sed "s/\]\]/$azulescuro \]\]<\/a>/" | sed "s/))/$azulescuro ))<\/a>/" | sed "s/esac/$rosa esac<\/a>/" | sed "s/done/$rosa done<\/a>/" | sed "s/;;/$azulescuro ;;<\/a>/" | sed "s/{/$azulescuro {<\/a>/" | sed "s/}/$azulescuro }<\/a>/" | sed "s/fi /$rosa fi<\/a>/" | sed "s/==/$rosa ==<\/a>/"  | sed 's/^/<h3>/' | sed 's/$/<\/h3>/' >> $render.html
			echo "$inferior" >> $render.html
			wkhtmltoimage --quality 100 $render.html $render.jpg
			ShellBot.sendChatAction --chat_id ${message_chat_id[$id]} --action upload_photo
			ShellBot.sendPhoto --chat_id ${message_chat_id[$id]} --photo @$render.jpg
